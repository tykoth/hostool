...
**/
	function prevPage($text='prev',$escapeTitle=true, $button=false)
	{
		if (empty($this->_pageDetails)) { return false; }
		if ( !empty($this->_pageDetails['previousPage']) )
		{
            return ($button) ? 
            $this->_generateButton($text, $this->_pageDetails['previousPage'], 'prev') : 
            $this->_generateLink($text,$this->_pageDetails['previousPage'],$escapeTitle);
		}
		return ($button) ?
		$this->_generateButton($text, null, 'prev disabled') :
		$text;
	}
...
**/
	function nextPage($text='next',$escapeTitle=true, $button=false)
	{
		if (empty($this->_pageDetails)) { return false; }
		if (!empty($this->_pageDetails['nextPage']))
		{
            return ($button) ? 
            $this->_generateButton($text, $this->_pageDetails['nextPage'], 'next') : 
            $this->_generateLink($text,$this->_pageDetails['nextPage'],$escapeTitle);
		}
		return ($button) ?
		$this->_generateButton($text, null, 'next disabled') :
		$text;
	}
...
* @return string html for link/text for next item
**/
	function firstPage($text='first',$escapeTitle=true, $button=false)
	{
		if (empty($this->_pageDetails)) { return false; }
		if ($this->_pageDetails['page']<>1)
		{
        	return ($button) ? 
            $this->_generateButton($text, 1, 'first') : 
            $this->_generateLink($text,1,$escapeTitle);
		}
		else
		{
			// @old return false
			return ($button) ?
			$this->_generateButton($text, null, 'first disabled') :
			$text;
		}
	}
...
* @return string html for link/text for next item
**/
	function lastPage($text='last',$escapeTitle=true, $button=false)
	{
		if (empty($this->_pageDetails)) { return false; }
		if ($this->_pageDetails['page']<>$this->_pageDetails['pageCount'])
		{
	        return ($button) ? 
            $this->_generateButton($text, $this->_pageDetails['pageCount'], 'last') : 
            $this->_generateLink($text,$this->_pageDetails['pageCount'],$escapeTitle);
		}
		else
		{
			// @old return false
			return ($button) ?
			$this->_generateButton($text, null, 'last disabled') :
			$text;
		}
	}
...
**/
	...
	function _generateButton($title = "", $page=NULL, $classes = "", $pageDetails = NULL)
	{
		$pageDetails = $pageDetails?$pageDetails:$this->_pageDetails;
		$url = $this->_generateUrl($page,$pageDetails);
		$url = $this->Html->url("/") . $url;
		
		return "<button class=\"{$classes}\" go=\"{$url}\">{$title}</button>";
	}
/**
